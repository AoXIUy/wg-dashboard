...
<div id="app">
            ...
            <header
                ...
                <div class="flex items-center gap-4 w-full md:w-auto justify-between md:justify-start">
                    <div class="flex items-center gap-4">
                        <div class="relative group cursor-pointer" @click="toggleTheme">
                            <div
                                class="absolute inset-0 bg-blue-500 blur-lg opacity-20 rounded-full group-hover:opacity-40 transition-opacity duration-500">
                            </div>
                            <div
                                class="bg-gradient-to-br from-white to-slate-50 dark:from-slate-800 dark:to-slate-900 p-2.5 rounded-xl border border-slate-200 dark:border-white/10 relative z-10 shadow-sm group-active:scale-95 transition-transform">
                        ...
                        <div>
                            ...
                            <div
                                ...
                                <span
                                    :class="['w-1.5 h-1.5 rounded-full transition-colors duration-500', isConnected ? 'bg-emerald-500 status-pulse' : 'bg-red-500']"></span>
                                {{ meta.interface }}:{{ meta.port }}
                                <span v-if="!isConnected" class="text-red-500 ml-1 animate-pulse">(连接中断)</span>
                            </div>
                        </div>
                    </div>

                    <!-- Mobile Menu Button (Optional, if we want to collapse actions) -->
                </div>
                ...
                <div class="flex flex-wrap items-center gap-2 w-full md:w-auto justify-end">
                    ...
                    <button @click="openDiagnostics"
                        class="p-2.5 rounded-xl hover:bg-teal-50 dark:hover:bg-teal-500/10 text-slate-500 dark:text-slate-400 hover:text-teal-600 dark:hover:text-teal-400 transition-colors"
                        title="网络诊断">
                                        ...
                                        <div class="flex flex-col gap-1">
                                            <div class="flex items-center gap-2">
                                                <div @click="copyText(p.allowed_ips[0], 'IP')"
                                                    class="bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded border border-slate-200 dark:border-white/5 w-fit font-mono text-xs text-slate-600 dark:text-slate-300 cursor-pointer hover:border-blue-400 hover:text-blue-500 transition-all truncate max-w-[120px]"
                                                    :title="p.allowed_ips[0]">
                                                    {{ p.allowed_ips[0] }}
                                                </div>
                                                <button @click.stop="openIpInfo(p.allowed_ips[0])"
                                                    class="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-400 hover:text-blue-500 transition-colors">
                                            ...
                                            <div class="flex items-center gap-2">
                                                ...
                                                <button v-if="p.endpoint" @click.stop="openIpInfo(p.endpoint)"
                                                    class="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-400 hover:text-blue-500 transition-colors">
                ...
                <div class="md:hidden p-4 space-y-4 flex-1 overflow-y-auto">
                    <transition-group name="list">
                        <div v-for="p in sortedPeers" :key="p.public_key"
                            class="glass-card rounded-2xl p-4 active:scale-[0.99] transition-all duration-200 group">
                            <div class="flex justify-between items-start mb-4">
                                <div class="flex items-center gap-3 overflow-hidden">
                                    <div
                                        :class="['w-2.5 h-2.5 rounded-full shrink-0', p.is_online ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)]' : 'bg-slate-300 dark:bg-slate-600']">
                                    </div>
                                    <div class="flex flex-col min-w-0">
                                        <div class="font-bold text-slate-800 dark:text-white flex items-center gap-2 truncate text-base"
                                            @click="editAlias(p)">
                                            {{ p.alias || '未命名' }}
                                        </div>
                                        <div class="flex items-center gap-2 mt-0.5">
                                            <div
                                                class="text-[10px] text-slate-500 font-mono bg-slate-100/80 dark:bg-slate-900/50 px-1.5 py-0.5 rounded border border-slate-200 dark:border-white/5 truncate">
                                                {{ p.allowed_ips[0] }}
                                            </div>
                                        </div>
                                    </div>
                                ...
                                <div class="flex gap-1 shrink-0">
                                    <button @click="openDetail(p)"
                                        class="p-2 -mt-2 -mr-1 text-slate-400 hover:text-blue-500 hover:bg-blue-50 dark:hover:bg-blue-900/20 rounded-lg transition-colors">
                            ...
                            <div
                                class="grid grid-cols-2 gap-3 bg-slate-50/50 dark:bg-slate-900/30 rounded-xl p-3 border border-slate-100 dark:border-white/5 mb-3">
                                <div>
                                    <div class="text-[10px] text-slate-500 mb-1 flex items-center gap-1"><i
                                            data-lucide="arrow-down" class="w-3 h-3"></i> 下载速率</div>
                                    <div class="text-emerald-600 dark:text-emerald-400 font-mono text-sm font-bold">{{
                                        fmtRate(p.rx_rate) }}</div>
                                </div>
                                <div class="text-right">
                                    <div class="text-[10px] text-slate-500 mb-1 flex items-center gap-1 justify-end">
                                        上传速率 <i data-lucide="arrow-up" class="w-3 h-3"></i></div>
                            ...
                            <div class="flex justify-between items-center pt-1">
                                <span class="text-[10px] text-slate-400">{{ p.endpoint ? p.endpoint.split(':')[0] :
                                    '未连接' }}</span>
        ...
        <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95 blur-sm"
            ...
            <div v-if="showAnalysis"
                ...
                <div
                    class="glass-card bg-white dark:bg-[#0f172a] rounded-3xl w-full max-w-6xl flex flex-col max-h-[90vh] shadow-2xl overflow-hidden">
                    <div
                        class="flex flex-col border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50">
                        <div class="flex justify-between items-center p-5">
                            <h2 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <i data-lucide="pie-chart" class="w-5 h-5 text-blue-500"></i> 网络深度分析
                            </h2>
                            <button @click="showAnalysis=false"
                                class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500"><i
                                    data-lucide="x" class="w-5 h-5"></i></button>
                        </div>
                        <div class="flex px-5 gap-1 overflow-x-auto">
                            <button @click="changeAnalysisTab('overview')"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', analysisTab==='overview' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">概览</button>
                            <button @click="changeAnalysisTab('map')"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', analysisTab==='map' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">全球分布</button>
                            <button @click="changeAnalysisTab('topology')"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', analysisTab==='topology' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">拓扑结构</button>
                            <button @click="changeAnalysisTab('heatmap')"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', analysisTab==='heatmap' ? 'border-blue-500 text-blue-600 dark:text-blue-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">流量热力</button>
                        </div>
                    </div>

                    <div class="p-0 flex-1 bg-slate-50/30 dark:bg-slate-900/20 relative flex flex-col overflow-hidden">
                        <div v-if="analysisLoading"
                            class="absolute inset-0 flex flex-col items-center justify-center z-10 bg-white/50 dark:bg-slate-900/50 backdrop-blur-sm">
                            <div
                                class="animate-spin w-8 h-8 border-2 border-blue-500 border-t-transparent rounded-full mb-3">
                            </div>
                            <span class="text-slate-500 text-sm">正在分析网络数据...</span>
                        </div>

                        <div v-show="analysisTab==='overview'" class="p-6 h-full overflow-y-auto custom-scrollbar">
                                            ...
                                            <tbody
                                                ...
                                                <tr v-for="p in analysisData.peers" :key="p.public_key"
                                                    ...
                                                    <td class="p-4 font-mono text-xs">{{
                                                        fmtBytes(p.total_rx+p.total_tx) }}
                                                    </td>
                                                    <td class="p-4">
                                                        <div class="flex items-center gap-2">
                                                            <div
                                                    ...
                                                    <td class="p-4">
                                                        <span class="px-2 py-0.5 rounded text-xs font-bold border"
                                                            :class="getHealthClass(p.health_score)">{{
                                                            p.health_score
                                                            }}</span>
                                                    </td>
                        ...
                        <div v-show="analysisTab==='map'" class="relative w-full h-full min-h-[500px]">
                            <div id="worldMap" class="absolute inset-0 z-0"></div>
                        </div>

                        <div v-show="analysisTab==='topology'"
                            class="relative w-full flex-1 flex overflow-hidden bg-slate-50 dark:bg-[#0b1121] min-h-[500px]">

                            <div
                                class="absolute inset-0 pointer-events-none opacity-30 bg-[radial-gradient(#cbd5e1_1px,transparent_1px)] dark:bg-[radial-gradient(#1e293b_1px,transparent_1px)] [background-size:20px_20px]">
                            </div>

                            <div class="absolute top-4 left-4 z-10 flex flex-col gap-2">
                                <div
                                    class="glass-card p-1.5 rounded-lg shadow-lg flex flex-col gap-1 bg-white/80 dark:bg-slate-800/80 backdrop-blur-md border border-slate-200 dark:border-white/10">
                                    <button @click="resetTopologyZoom" title="重置视图"
                                        class="p-2 hover:bg-slate-100 dark:hover:bg-white/10 rounded-md text-slate-500 transition-colors">
                                        <i data-lucide="maximize" class="w-5 h-5"></i>
                                    </button>
                                    <button @click="togglePhysics" title="冻结/解冻布局"
                                        :class="['p-2 rounded-md transition-colors', physicsEnabled ? 'text-blue-500 bg-blue-50 dark:bg-blue-500/20' : 'text-slate-500 hover:bg-slate-100 dark:hover:bg-white/10']">
                            ...
                            <div class="absolute bottom-4 left-4 z-10">
                                <div
                                    class="glass-card px-4 py-3 rounded-xl shadow-lg bg-white/90 dark:bg-slate-800/90 backdrop-blur-md border border-slate-200 dark:border-white/10 text-xs font-medium text-slate-600 dark:text-slate-300 space-y-2">
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="w-3 h-3 rounded-full bg-blue-500 shadow-[0_0_8px_rgba(59,130,246,0.5)]"></span>
                                        <span>服务器 (Server)</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                                        <span
                                            class="w-3 h-3 rounded-full bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.5)]"></span>
                                        <span>在线客户端</span>
                                    </div>
                                    <div class="flex items-center gap-2">
                            ...
                            <div id="networkTopology"
                                class="absolute inset-0 w-full h-full outline-none active:cursor-grabbing"></div>
                            ...
                            <Transition enter-active-class="transform transition ease-in-out duration-300"
                                ...
                                <div v-if="selectedNode"
                                    class="absolute right-0 top-0 h-full w-80 bg-white/95 dark:bg-[#0f172a]/95 backdrop-blur-xl border-l border-slate-200 dark:border-white/10 shadow-2xl z-20 p-6 overflow-y-auto custom-scrollbar">
                                    ...
                                    <div class="flex justify-between items-start mb-5">
                                        ...
                                        <button @click="selectedNode=null"
                                            class="p-1.5 hover:bg-slate-100 dark:hover:bg-white/10 rounded-lg text-slate-400 transition-colors">
                                    ...
                                    <div class="space-y-4">
                                        <!-- 状态卡片 -->
                                        <div class="p-4 rounded-xl bg-gradient-to-br border text-center"
                                            :class="selectedNode.online ? 'from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/10 border-emerald-200 dark:border-emerald-500/30' : 'from-slate-50 to-slate-100 dark:from-slate-800/30 dark:to-slate-900/30 border-slate-200 dark:border-slate-700/30'">
                                            <div class="flex items-center justify-center gap-2 mb-2">
                                                <span class="w-2.5 h-2.5 rounded-full"
                                                    :class="selectedNode.online ? 'bg-emerald-500 shadow-[0_0_8px_rgba(16,185,129,0.6)] animate-pulse' : 'bg-slate-400'"></span>
                                                <div class="text-2xl font-bold"
                                                    :class="selectedNode.online ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-500 dark:text-slate-400'">
                                                    {{ selectedNode.online ? '在线' : '离线' }}
                                                </div>
                                            </div>
                                            <div class="text-xs font-medium text-slate-600 dark:text-slate-300 mb-1">{{
                                                selectedNode.alias }}</div>
                                            <div class="text-xs text-slate-400 font-mono">{{ selectedNode.ip }}</div>
                                        </div>

                                        <!-- 实时速率 -->
                                        <div v-if="selectedNode.online" class="grid grid-cols-2 gap-3">
                                            <div
                                                class="p-3 rounded-xl bg-gradient-to-br from-blue-50 to-indigo-50 dark:from-blue-900/20 dark:to-indigo-900/10 border border-blue-100 dark:border-blue-500/20">
                                                <div class="flex items-center gap-1.5 mb-1">
                                                    <i data-lucide="arrow-up" class="w-3 h-3 text-blue-500"></i>
                                                    <div
                                                        class="text-[10px] text-blue-600 dark:text-blue-400 font-semibold uppercase">
                                                        上传</div>
                                                </div>
                                                <div
                                                    class="text-lg font-bold font-mono text-blue-600 dark:text-blue-400">
                                                    {{ fmtRate(selectedNode.txRate) }}
                                                </div>
                                            </div>
                                            <div
                                                class="p-3 rounded-xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/10 border border-emerald-100 dark:border-emerald-500/20">
                                                <div class="flex items-center gap-1.5 mb-1">
                                                    <i data-lucide="arrow-down" class="w-3 h-3 text-emerald-500"></i>
                                                    <div
                                                        class="text-[10px] text-emerald-600 dark:text-emerald-400 font-semibold uppercase">
                                                        下载</div>
                                                </div>
                                                <div
                                                    class="text-lg font-bold font-mono text-emerald-600 dark:text-emerald-400">
                                                    {{ fmtRate(selectedNode.rxRate) }}
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <!-- 累计流量 -->
                                        <div
                                            class="space-y-2 p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-white/5">
                                            <div class="text-[10px] text-slate-400 uppercase font-bold mb-2">累计流量</div>
                                            <div class="flex justify-between items-center text-xs">
                                                <span class="text-slate-500">上传 (Tx)</span>
                                                <span class="font-mono font-bold text-blue-600 dark:text-blue-400">{{
                                                    fmtBytes(selectedNode.tx) }}</span>
                                            </div>
                                            <div class="flex justify-between items-center text-xs">
                                                <span class="text-slate-500">下载 (Rx)</span>
                                                <span
                                                    class="font-mono font-bold text-emerald-600 dark:text-emerald-400">{{
                                                    fmtBytes(selectedNode.rx) }}</span>
                                            </div>
                                        </div>
                                        ...
                                        <div
                                            class="space-y-2 p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-white/5">
                                            <div class="text-[10px] text-slate-400 uppercase font-bold mb-2">连接信息</div>
                                            <div class="space-y-2 text-xs">
                                                <div class="flex justify-between items-center">
                                                    <span class="text-slate-500">端点</span>
                                                    <span
                                                        class="font-mono text-slate-700 dark:text-slate-200 truncate max-w-[160px]"
                                                        :title="selectedNode.endpoint">
                                                        {{ selectedNode.endpoint }}
                                                    </span>
                                                </div>
                                                <div class="flex justify-between items-center">
                                                    <span class="text-slate-500">最后握手</span>
                                                    <span class="text-slate-700 dark:text-slate-200">
                                                        {{ selectedNode.latestHandshake ?
                                                        formatLastSeen(selectedNode.latestHandshake) : '从未' }}
                                                    </span>
                                                </div>
                                                <div class="flex justify-between items-center">
                                                    <span class="text-slate-500">最后活跃</span>
                                                    <span class="text-slate-700 dark:text-slate-200">
                                        ...
                                        <!-- 公钥 (可复制) -->
                                        <div
                                            class="p-3 bg-slate-50 dark:bg-slate-800/30 rounded-xl border border-slate-100 dark:border-white/5">
                                            <div class="flex items-center justify-between mb-2">
                                                <div class="text-[10px] text-slate-400 uppercase font-bold">公钥</div>
                                                <button @click="copyText(selectedNode.fullKey, '公钥')"
                                                    class="p-1 hover:bg-slate-200 dark:hover:bg-slate-700 rounded text-slate-400 hover:text-blue-500 transition-colors"
                                                    title="复制公钥">
                                                    <i data-lucide="copy" class="w-3 h-3"></i>
                                                </button>
                                            </div>
                                            <div
                                                class="text-[10px] font-mono text-slate-600 dark:text-slate-400 break-all leading-relaxed bg-white dark:bg-slate-900/50 p-2 rounded border border-slate-200 dark:border-white/5">
                        ...
                        <div v-show="analysisTab==='heatmap'"
                            class="p-6 lg:p-10 space-y-8 h-full overflow-y-auto custom-scrollbar">

                            <div
                                class="glass-card bg-white dark:bg-[#0f172a] rounded-3xl p-6 shadow-xl border border-slate-100 dark:border-white/5 relative overflow-hidden">
                                <div
                                    class="absolute -top-20 -right-20 w-64 h-64 bg-blue-500/10 rounded-full blur-3xl pointer-events-none">
                                </div>

                                <div
                                    class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 relative z-10">
                                    <div>
                                        <h3
                                            class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                        ...
                                        <p class="text-xs text-slate-500 mt-1">识别网络使用高峰时段与空闲窗口</p>
                                    </div>

                                    <div
                                        class="flex items-center gap-2 text-xs text-slate-500 mt-4 sm:mt-0 bg-slate-50 dark:bg-slate-800/50 p-2 rounded-lg border border-slate-100 dark:border-white/5">
                                        <span>低负载</span>
                                        <div
                                            class="w-24 h-2 bg-gradient-to-r from-blue-100 via-blue-500 to-red-500 rounded-full">
                                        </div>
                                        <span>高负载</span>
                                    </div>
                                </div>

                                <div class="relative w-full h-[320px] z-10">
                                    <canvas id="trafficHeatmap"></canvas>
                                </div>
                            </div>

                            <div v-if="analysisData && analysisData.optimal_time"
                                class="grid grid-cols-1 md:grid-cols-2 gap-6">

                                <div
                                    class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/10 p-6 border border-emerald-100 dark:border-emerald-500/20">
                                    <div
                                        class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                        <i data-lucide="clock" class="w-24 h-24 text-emerald-600"></i>
                                    </div>

                                    <div class="relative z-10">
                                        <div class="flex items-center gap-3 mb-3">
                                            <div
                                                class="p-2 bg-emerald-100 dark:bg-emerald-500/20 rounded-lg text-emerald-600 dark:text-emerald-400">
                                                <i data-lucide="wrench" class="w-5 h-5"></i>
                                            </div>
                                            <h4 class="font-bold text-emerald-900 dark:text-emerald-100">最佳维护时段</h4>
                                        </div>
                                        <div class="text-3xl font-bold text-emerald-700 dark:text-emerald-400 mb-1">
                                            {{analysisData.optimal_time.start_hour}}:00 -
                                            {{analysisData.optimal_time.end_hour}}:00
                                        </div>
                                        <p class="text-sm text-emerald-600/80 dark:text-emerald-400/70">
                                            该时段全网平均负载最低，仅为
                                            <span
                                                class="font-mono font-bold">{{fmtRate(analysisData.optimal_time.avg_load_mbps)}}</span>，
                                            适合进行服务器重启或配置更新。
                                        </p>
                                    </div>
                                </div>
                                ...
                                <div
                                    class="group relative overflow-hidden rounded-2xl bg-gradient-to-br from-amber-50 to-orange-50 dark:from-amber-900/20 dark:to-orange-900/10 p-6 border border-amber-100 dark:border-amber-500/20">
                                    <div
                                        class="absolute top-0 right-0 p-4 opacity-10 group-hover:opacity-20 transition-opacity">
                                        <i data-lucide="alert-triangle" class="w-24 h-24 text-amber-600"></i>
                                    </div>

                                    <div class="relative z-10">
                                        <div class="flex items-center gap-3 mb-3">
                                            <div
                                                class="p-2 bg-amber-100 dark:bg-amber-500/20 rounded-lg text-amber-600 dark:text-amber-400">
                                                <i data-lucide="activity" class="w-5 h-5"></i>
                                            </div>
                                            <h4 class="font-bold text-amber-900 dark:text-amber-100">峰值预警</h4>
                                        </div>
                                        <div class="text-3xl font-bold text-amber-700 dark:text-amber-400 mb-1">
                                            14:00 - 16:00
                                        </div>
                                        <p class="text-sm text-amber-600/80 dark:text-amber-400/70">
                                            检测到工作日下午出现流量高峰，建议在此期间避免进行带宽密集型操作。
                                        </p>
                                    </div>
                                </div>
                            </div>
        ...
        <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95 blur-sm"
            ...
            <div v-if="showDiagnostics"
                ...
                <div
                    class="glass-card bg-white dark:bg-[#0f172a] rounded-3xl w-full max-w-6xl flex flex-col h-[90vh] shadow-2xl overflow-hidden">
                    <div
                        class="flex flex-col border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/50 shrink-0">
                        <div class="flex justify-between items-center p-5">
                            <h2 class="text-lg font-bold text-slate-800 dark:text-white flex items-center gap-2">
                                <i data-lucide="terminal" class="w-5 h-5 text-teal-500"></i> 网络诊断工具
                            </h2>
                            <button @click="showDiagnostics=false"
                                class="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500">
                                <i data-lucide="x" class="w-5 h-5"></i>
                            </button>
                        </div>
                        <div class="flex px-5 gap-1 overflow-x-auto">
                            <button @click="diagTab='ping'"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', diagTab==='ping' ? 'border-teal-500 text-teal-600 dark:text-teal-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">Ping</button>
                            <button @click="diagTab='traceroute'"
                                :class="['px-4 py-2 text-sm font-medium border-b-2 transition-colors whitespace-nowrap', diagTab==='traceroute' ? 'border-teal-500 text-teal-600 dark:text-teal-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:hover:text-slate-300']">Traceroute</button>
                        </div>
                    </div>

                    <div class="flex-1 overflow-y-auto custom-scrollbar p-6 bg-slate-50/30 dark:bg-slate-900/20">
                        <div v-show="diagTab==='ping'" class="flex flex-col lg:flex-row gap-6 h-full">
                            <div class="w-full lg:w-1/3 glass-card rounded-2xl p-6 h-fit shrink-0">
                                <h3 class="font-bold text-slate-800 dark:text-white mb-4">目标配置</h3>
                                <div class="space-y-4">
                                    <div>
                                        <label class="block text-xs text-slate-500 mb-1">选择客户端 IP</label>
                                        <select v-model="diagTarget"
                                            class="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-teal-500">
                                            <option value="">手动输入...</option>
                                            <option v-for="p in peers" :key="p.public_key"
                                                :value="p.endpoint ? extractEndpointIP(p.endpoint) : p.allowed_ips[0]">
                                                {{ p.alias || p.public_key.substring(0,8) }} ({{ p.endpoint ?
                                                extractEndpointIP(p.endpoint) : p.allowed_ips[0] }})
                                            </option>
                                        </select>
                                    </div>
                                    <div class="flex items-center gap-4">
                                        <div class="h-[1px] flex-1 bg-slate-200 dark:bg-slate-700"></div>
                                        <span class="text-xs text-slate-400">或者</span>
                                        <div class="h-[1px] flex-1 bg-slate-200 dark:bg-slate-700"></div>
                                    </div>
                                    <div>
                                        <label class="block text-xs text-slate-500 mb-1">自定义 IP / 域名</label>
                                        <input v-model="diagTarget" type="text" placeholder="例如: 8.8.8.8"
                                            class="w-full bg-slate-100 dark:bg-slate-800 border-none rounded-xl px-4 py-2.5 text-sm outline-none focus:ring-2 focus:ring-teal-500"
                                            @keyup.enter="executePing">
                                    </div>
                                    <div>
                                        <label class="block text-xs text-slate-500 mb-1">Ping 次数</label>
                                        <div class="flex items-center gap-3">
                                            <button @click="diagCount > 1 && diagCount--"
                                                class="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700"><i
                                                    data-lucide="minus" class="w-4 h-4"></i></button>
                                            <span
                                                class="font-mono text-sm font-bold w-4 text-center text-slate-700 dark:text-slate-300">{{
                                                diagCount }}</span>
                                            <button @click="diagCount < 20 && diagCount++"
                                                class="p-2 bg-slate-100 dark:bg-slate-800 rounded-lg hover:bg-slate-200 dark:hover:bg-slate-700"><i
                                                    data-lucide="plus" class="w-4 h-4"></i></button>
                                        </div>
                                    </div>
                                    <button @click="executePing" :disabled="diagLoading || !diagTarget"
                                        class="w-full mt-4 bg-teal-600 hover:bg-teal-500 text-white font-bold py-3 rounded-xl transition-all shadow-lg shadow-teal-600/20 active:scale-95 disabled:opacity-50 flex justify-center items-center gap-2">
                                        <!-- loading: 旋转圈 -->
                                        <svg v-if="diagLoading" class="w-4 h-4 animate-spin"
                                            xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            stroke="currentColor" stroke-width="2">
                                            <path stroke-linecap="round" stroke-linejoin="round"
                                                d="M12 3v3m0 12v3M3 12h3m12 0h3m-2.636-6.364-2.122 2.122M6.758 17.243l-2.121 2.121M17.364 17.364l-2.122-2.122M6.757 6.757 4.636 4.636" />
                                        </svg>
                                        <!-- idle: play 图标 -->
                            ...
                            <div class="flex-1 flex flex-col gap-4">
                                <div v-if="!pingResult && !diagLoading"
                                    class="flex-1 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center text-slate-400">
                                    <i data-lucide="activity" class="w-12 h-12 mb-2 opacity-50"></i>
                                    <p class="text-sm">配置并在左侧开始执行诊断</p>
                                </div>
                                <div v-if="diagLoading" class="animate-pulse space-y-4">
                                    <div class="h-32 bg-slate-200 dark:bg-slate-800 rounded-2xl"></div>
                                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                        <div class="h-24 bg-slate-200 dark:bg-slate-800 rounded-2xl" v-for="i in 4">
                                        </div>
                                    </div>
                                </div>
                                <div v-if="pingResult && !diagLoading" class="space-y-4">
                                    <div class="glass-card rounded-2xl p-6 bg-gradient-to-br"
                                        :class="pingResult.is_alive ? 'from-emerald-50 to-teal-50 dark:from-emerald-900/20 dark:to-teal-900/10 border-emerald-200 dark:border-emerald-500/30' : 'from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/10 border-red-200 dark:border-red-500/30'">
                                        <div class="flex flex-col sm:flex-row justify-between sm:items-start gap-4">
                                            <div>
                                                <div class="text-xs font-bold uppercase mb-1"
                                                    :class="pingResult.is_alive ? 'text-emerald-600' : 'text-red-500'">
                                                    {{ pingResult.is_alive ? '在线 (Host Alive)' : '离线 (Host Unreachable)'
                                                    }}</div>
                                                <div
                                                    class="text-2xl font-mono font-bold text-slate-800 dark:text-white">
                                                    {{ pingResult.address }}</div>
                                            </div>
                                            <div v-if="pingResult.country_code"
                                                class="flex items-center gap-2 bg-white/50 dark:bg-black/20 p-2 rounded-xl border border-white/20 w-fit">
                                                <img :src="`https://flagcdn.com/w40/${pingResult.country_code.toLowerCase()}.png`"
                                                    class="w-6 rounded shadow-sm">
                                                <span class="text-sm font-medium text-slate-800 dark:text-white">{{
                                                    pingResult.city }}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4">
                                        <div class="glass-card rounded-2xl p-4 bg-white dark:bg-slate-800/50"
                                            style="animation-delay: 50ms;">
                                            <div class="text-[10px] text-slate-400 font-bold mb-1">平均延迟 (Avg RTT)</div>
                                            <div class="text-xl font-mono font-bold text-blue-500">{{ pingResult.avg_rtt
                                                }}<span class="text-sm ml-1 text-slate-500">ms</span></div>
                                        </div>
                                        <div class="glass-card rounded-2xl p-4 bg-white dark:bg-slate-800/50"
                                            style="animation-delay: 100ms;">
                                            <div class="text-[10px] text-slate-400 font-bold mb-1">最小延迟 (Min RTT)</div>
                                            <div class="text-xl font-mono font-bold text-teal-500">{{ pingResult.min_rtt
                                                }}<span class="text-sm ml-1 text-slate-500">ms</span></div>
                                        </div>
                                        <div class="glass-card rounded-2xl p-4 bg-white dark:bg-slate-800/50"
                                            style="animation-delay: 150ms;">
                                            <div class="text-[10px] text-slate-400 font-bold mb-1">丢包率 (Loss)</div>
                                            <div class="text-xl font-mono font-bold"
                                                :class="pingResult.packet_loss > 0 ? 'text-red-500' : 'text-emerald-500'">
                                                {{ pingResult.packet_loss }}<span
                                                    class="text-sm ml-1 text-slate-500">%</span></div>
                                        </div>
                                        <div class="glass-card rounded-2xl p-4 bg-white dark:bg-slate-800/50"
                                            style="animation-delay: 200ms;">
                                            <div class="text-[10px] text-slate-400 font-bold mb-1">收发包 (Rx/Tx)</div>
                                            <div class="text-xl font-mono font-bold text-slate-700 dark:text-slate-300">
                                                {{ pingResult.packets_received }}/{{ pingResult.packets_sent }}</div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div v-show="diagTab==='traceroute'" class="flex flex-col gap-6 h-full">
                            <div
                                class="glass-card rounded-2xl p-4 flex gap-4 items-center shrink-0 bg-white dark:bg-slate-800/50">
                                <div class="relative flex-1 group">
                                    <i data-lucide="search"
                                        class="absolute left-4 top-1/2 -translate-y-1/2 w-5 h-5 text-slate-400 group-focus-within:text-teal-500 transition-colors"></i>
                                    <input v-model="diagTarget" type="text" placeholder="输入目标 IP 或域名以追踪路由..."
                                        class="w-full bg-slate-50 dark:bg-slate-900/80 border border-slate-200 dark:border-slate-700 text-slate-800 dark:text-white rounded-xl pl-12 pr-4 py-3 focus:ring-2 focus:ring-teal-500 outline-none transition-all"
                                        @keyup.enter="executeTraceroute">
                                </div>
                                <button @click="executeTraceroute" :disabled="diagLoading || !diagTarget"
                                    class="px-6 py-3 bg-teal-600 hover:bg-teal-500 text-white font-bold rounded-xl transition-all shadow-lg shadow-teal-600/20 active:scale-95 disabled:opacity-50 flex items-center gap-2">
                                    <!-- loading: 旋转圈 -->
                                    <svg v-if="diagLoading" class="w-4 h-4 animate-spin"
                                        xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                        stroke="currentColor" stroke-width="2">
                            ...
                            <div v-if="!tracerouteResult && !diagLoading"
                                class="flex-1 border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-2xl flex flex-col items-center justify-center text-slate-400">
                                <i data-lucide="git-merge" class="w-12 h-12 mb-2 opacity-50"></i>
                                <p class="text-sm">执行 Traceroute 后将在这里展示路径 (最多 30 跳)</p>
                            </div>

                            <div v-if="diagLoading" class="animate-pulse space-y-2">
                                <div class="h-12 bg-slate-200 dark:bg-slate-800 rounded-xl w-full" v-for="i in 5"></div>
                            </div>

                            <div v-if="tracerouteResult && !diagLoading"
                                class="flex-1 glass-card rounded-2xl overflow-hidden bg-white dark:bg-slate-800/50 flex flex-col">
                                <div
                                    class="p-4 border-b border-slate-100 dark:border-white/5 bg-slate-50/50 dark:bg-slate-900/30 font-mono text-sm text-slate-600 dark:text-slate-400 font-bold">
                                    Traceroute to <span class="text-teal-500">{{ tracerouteResult.target }}</span>
                                </div>
                                <div class="flex-1 overflow-auto custom-scrollbar">
                                    <table class="w-full text-left text-sm">
                                        <thead
                                            class="bg-slate-50 dark:bg-slate-900/50 text-xs text-slate-500 dark:text-slate-400 border-b border-slate-100 dark:border-white/5 sticky top-0 z-10">
                                            <tr>
                                                <th class="p-4 font-bold">跳数 (Hop)</th>
                                                <th class="p-4 font-bold">IP 地址</th>
                                                <th class="p-4 font-bold">延迟 (ms)</th>
                                                <th class="p-4 font-bold">地理位置</th>
                                            </tr>
                                        </thead>
                                        <tbody class="divide-y divide-slate-100 dark:divide-white/5 font-mono">
                                            <tr v-for="hop in tracerouteResult.hops" :key="hop.distance"
                                                class="hover:bg-slate-50/80 dark:hover:bg-white/5 transition-colors">
                                                <td class="p-4 text-slate-400 w-20">{{ hop.distance }}</td>
                                                <td class="p-4">
                                                    <div class="flex items-center gap-2">
                                                        <span
                                                            :class="hop.address === '*' ? 'text-red-400' : 'text-slate-800 dark:text-slate-200 font-bold'">{{
                                                            hop.address === '*' ? 'Request timed out' : hop.address
                                                            }}</span>
                                                    </div>
                                                </td>
                                                <td class="p-4 w-32">
                                                    <span v-if="hop.address !== '*'"
                                                        class="text-emerald-600 dark:text-emerald-400">{{ hop.avg_rtt }}
                                                        ms</span>
                                                    <span v-else class="text-red-400">*</span>
                                                </td>
                                                <td class="p-4">
                                                    <div class="flex items-center gap-2 text-xs font-sans"
                                                        v-if="hop.country_code">
                                                        <img :src="`https://flagcdn.com/w20/${hop.country_code.toLowerCase()}.png`"
                                                            class="w-4 rounded shadow-sm inline">
                                                        <span class="text-slate-600 dark:text-slate-400">{{ hop.city ||
                                                            hop.country_code }}</span>
                                                    </div>
                                                    <span v-else-if="hop.address !== '*'"
                                                        class="text-slate-400 text-xs font-sans">-</span>
                                                </td>
                                            </tr>
                                        </tbody>
                                    </table>
                    ...
                    <div class="flex justify-between items-start mb-6">
                        <div class="flex items-center gap-4">
                            ...
                            <div>
                                ...
                                <div v-if="modalPeer.latency"
                                    class="mt-2 inline-flex items-center gap-1.5 px-2 py-0.5 rounded text-[10px] font-mono font-bold bg-emerald-50 text-emerald-600 dark:bg-emerald-500/10 dark:text-emerald-400 border border-emerald-100 dark:border-emerald-500/20">
                                    <i data-lucide="activity" class="w-3 h-3"></i> {{ modalPeer.latency }}
                                </div>
                            </div>
                        </div>
                                        ...
                                        <td class="p-3">
                                            <div class="flex items-center gap-2">
                                                <img v-if="getCountryCode(log.endpoint)"
                                                    :src="`https://flagcdn.com/w40/${getCountryCode(log.endpoint).toLowerCase()}.png`"
                                                    class="w-5 h-auto rounded shadow-sm object-cover"
                                                    :title="getGeoTitle(log.endpoint)">
                                                <span class="font-bold font-mono">{{ log.endpoint }}</span>
                                                <button @click.stop="openIpInfo(log.endpoint)"
                                                    class="p-1 rounded-md hover:bg-slate-200 dark:hover:bg-slate-700 text-slate-400 hover:text-blue-500 transition-colors"
                                                    title="详细信息">
                                                    <i data-lucide="info" class="w-3 h-3"></i>
                                                </button>
        ...
        <Transition enter-active-class="transition duration-300 ease-out" enter-from-class="opacity-0 scale-95 blur-sm"
            enter-to-class="opacity-100 scale-100 blur-0" leave-active-class="transition duration-200 ease-in"
            leave-from-class="opacity-100 scale-100" leave-to-class="opacity-0 scale-95 blur-sm">
            <div v-if="showIpDetailModal"
                class="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-50 flex items-center justify-center p-4"
                @click.self="showIpDetailModal=false">
                <div class="glass-card bg-white dark:bg-[#0f172a] rounded-3xl w-full max-w-sm p-6 shadow-2xl relative">
                    <button @click="showIpDetailModal=false"
                        class="absolute top-4 right-4 p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full text-slate-500">
                        <i data-lucide="x" class="w-5 h-5"></i>
                    </button>

                    <div class="text-center mb-6">
                        <div
                            class="inline-flex items-center justify-center w-12 h-12 rounded-xl bg-blue-50 dark:bg-blue-900/20 text-blue-500 mb-3">
                            <i data-lucide="globe" class="w-6 h-6"></i>
                        </div>
                        <h3 class="text-lg font-bold text-slate-800 dark:text-white">IP 信息详情</h3>
                    </div>

                    <div v-if="selectedIpInfo.loading" class="flex flex-col items-center justify-center py-8">
                        <i data-lucide="loader-2" class="w-8 h-8 animate-spin text-blue-500 mb-2"></i>
                        <span class="text-xs text-slate-400">正在查询 GeoIP 数据库...</span>
                    </div>

                    <div v-else class="space-y-4">
                        <div
                            class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-white/5 text-center">
                            <div class="text-xs text-slate-400 mb-1">IP 地址</div>
                            <div
                                class="text-xl font-mono font-bold text-slate-700 dark:text-slate-200 select-all break-all">
                                {{ selectedIpInfo.ip }}</div>
                            <div v-if="selectedIpInfo.isPrivate"
                                class="mt-2 inline-block px-2 py-0.5 rounded text-[10px] bg-slate-200 text-slate-600 dark:bg-slate-700 dark:text-slate-300">
                                局域网 / 私有地址
                            </div>
                        </div>

                        <div v-if="!selectedIpInfo.isPrivate" class="grid grid-cols-2 gap-3">
                            <div
                                class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-white/5">
                                <div class="text-[10px] text-slate-400 mb-1">国家 / 地区</div>
                                <div class="flex items-center gap-2 font-medium text-slate-700 dark:text-slate-200">
                                    <img v-if="selectedIpInfo.country_code"
                                        :src="`https://flagcdn.com/w40/${selectedIpInfo.country_code.toLowerCase()}.png`"
                                        class="w-5 rounded shadow-sm">
                                    <span>{{ selectedIpInfo.country_code || '-' }}</span>
                                </div>
                            </div>
                            <div
                                class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-3 border border-slate-100 dark:border-white/5">
                                <div class="text-[10px] text-slate-400 mb-1">城市</div>
                                <div class="font-medium text-slate-700 dark:text-slate-200 truncate"
                                    :title="selectedIpInfo.city">
                                    {{ selectedIpInfo.city || '-' }}
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="!selectedIpInfo.isPrivate"
                            class="bg-slate-50 dark:bg-slate-900/50 rounded-xl p-4 border border-slate-100 dark:border-white/5">
                            <div class="text-[10px] text-slate-400 mb-1">网络运营商 (ASN)</div>
                            <div class="text-sm font-medium text-slate-700 dark:text-slate-200 break-words">
                                <span v-if="selectedIpInfo.asn_number" class="text-blue-500 font-mono mr-1">AS{{
                                    selectedIpInfo.asn_number }}</span>
                                {{ selectedIpInfo.asn || '未知' }}
                            </div>
                        </div>
                    </div>
                </div>
...
const { createApp, ref, reactive, onMounted, onUnmounted, computed, nextTick, watch } = Vue;
        ...
        createApp({
            setup() {
                ...
                const diagTab = ref('ping');
                const diagTarget = ref('');
                const diagCount = ref(4);
                const diagLoading = ref(false);
                const pingResult = ref(null);
                const tracerouteResult = ref(null);

                // 从 endpoint 提取 IP，兼容 IPv4 ("1.2.3.4:port") 和 IPv6 ("[::1]:port")
                const extractEndpointIP = (endpoint) => {
                    if (!endpoint) return '';
                ...
                const executePing = async () => {
                    if (!diagTarget.value) return showToast('请输入目标 IP', 'error');
                    diagLoading.value = true;
                    pingResult.value = null;
                    nextTick(() => lucide.createIcons());
                    try {
                        const res = await axios.get('/api/ping/execute', {
                            params: { ipAddress: diagTarget.value, count: diagCount.value },
                            headers: { Authorization: `Bearer ${token.value}` }
                        });
                        pingResult.value = res.data;
                    } catch (e) {
                        showToast('Ping 执行失败', 'error');
                    } finally {
                ...
                const executeTraceroute = async () => {
                    ...
                    nextTick(() => lucide.createIcons());
                    try {
                        const res = await axios.get('/api/traceroute/execute', {
                            params: { ipAddress: diagTarget.value },
                            headers: { Authorization: `Bearer ${token.value}` }
                        });
                        tracerouteResult.value = res.data;
                    } catch (e) {
                        showToast('Traceroute 执行失败', 'error');
                    } finally {
                        diagLoading.value = false;
                        nextTick(() => lucide.createIcons());
                    }
                ...
                const resolveIpLocation = async (endpoint) => {
                    ...
                    // 设置临时状态避免重复请求
                    ipCache[ip] = { loading: true };

                    try {
                        const res = await axios.get(`/api/geoip?ip=${ip}`, { headers: { Authorization: `Bearer ${token.value}` } });
                        ipCache[ip] = res.data;
                    } catch (e) {
                        delete ipCache[ip];
                    }
                ...
                const getCountryCode = (endpoint) => {
                    const data = ipCache[getIp(endpoint)];
                    return (data && !data.loading) ? data.country_code : '';
                };

                const getGeoTitle = (endpoint) => {
                    const data = ipCache[getIp(endpoint)];
                    if (!data || data.loading) return endpoint;
                    let parts = [];
                    if (data.country_code) parts.push(data.country_code);
                    if (data.city) parts.push(data.city);
                    if (data.asn) parts.push(`AS${data.asn_number} ${data.asn}`);
                    return parts.join(' - ') || endpoint;
                };

                const showIpDetailModal = ref(false);
                const selectedIpInfo = ref({ ip: '', isPrivate: false, loading: false });

                const openIpInfo = async (rawIp) => {
                    if (!rawIp) return;

                    // 清洗 IP (去除 CIDR 和 端口)
                    let ip = rawIp.split('/')[0]; // 去除掩码

                    if (ip.startsWith('[')) {
                        // IPv6 [xxx]:port
                        ip = ip.split(']:')[0].replace('[', '');
                    } else if (ip.includes(':')) {
                        // IPv4:Port (特征是只有一个冒号) vs IPv6 (多个冒号)
                    ...
                    selectedIpInfo.value = { ip, loading: true, isPrivate: false };

                    if (isPrivateIp(ip)) {
                        selectedIpInfo.value = { ip, isPrivate: true, loading: false };
                        return;
                    }

                    // 如果缓存里没有，先请求
                    if (!ipCache[ip] || ipCache[ip].loading) {
                        await resolveIpLocation(ip);
                    }

                    // 从缓存读取结果
                    const data = ipCache[ip];
                    if (data) {
                        selectedIpInfo.value = { ip, isPrivate: false, loading: false, ...data };
                    } else {
                        selectedIpInfo.value = { ip, loading: false, error: true };
                    }
                    nextTick(() => lucide.createIcons());
                };
                ...
                watch(isDark, () => {
                    ...
                    if (analysisTab.value === 'map') initEChartsMap();
                    if (analysisTab.value === 'topology') initTopology();
                    if (analysisTab.value === 'heatmap') initHeatmap();
                });
                ...
                const fetchDetailData = async () => {
                    ...
                    try {
                        const res = await axios.get(`/api/history?publickey=${encodeURIComponent(modalPeer.value.public_key)}&period=${historyPeriod.value}`, { headers: { Authorization: `Bearer ${token.value}` } });
                ...
                const fetchAccessLogs = async () => {
                    ...
                    accessLogs.value = [];
                    try {
                        const res = await axios.get(`/api/history/logs?publickey=${encodeURIComponent(modalPeer.value.public_key)}`, {
                            headers: { Authorization: `Bearer ${token.value}` }
                        });
                ...
                const openAnalysis = async () => {
                    ...
                    try {
                        ...
                        lucide.createIcons();
                        if (analysisTab.value === 'map') initMap();
                        if (analysisTab.value === 'topology') initTopology();
                    }
                };

                const analysisTab = ref('overview');
                const changeAnalysisTab = async (tab) => {
                    ...
                    if (tab === 'topology') {
                        // [修复] 增加延时，确保 Flex 布局计算完毕，容器有实际高度后再初始化
                        setTimeout(() => {
                            initTopology();
                        }, 100);
                    }
                ...
                const initMap = async () => {
                    try {
                        const res = await axios.get('/api/map/data', { headers: { Authorization: `Bearer ${token.value}` } });
                        window.AdvancedApp.initEChartsMap('worldMap', res.data);
                ...
                const initTopology = async () => {
                    ...
                    const nodes = [
                        {
                            ...
                            value: 20,
                            online: true,
                            ip: meta.value.interface,
                            tx: totalTxRate.value,
                            rx: totalRxRate.value
                        }
                    ];
                    ...
                    peers.value.forEach(p => {
                        nodes.push({
                            ...
                            group: p.is_online ? 'client_online' : 'client_offline',
                            value: 10,
                            online: p.is_online,
                            ip: p.allowed_ips[0],
                            tx: p.transmit_bytes,
                            ...
                            rxRate: p.rx_rate || 0,
                            // 新增字段：连接信息
                            endpoint: p.endpoint || '未连接',
                            latestHandshake: p.latest_handshake || 0,
                            // 完整公钥用于复制
                            fullKey: p.public_key,
                            // 别名
                            alias: p.alias || '未命名客户端'
                        });

                        edges.push({
                            from: 'server',
                            to: p.public_key,
                            value: p.is_online ? 2 : 1,
                            dashes: !p.is_online,
                            color: {
                                color: p.is_online ? (isDark.value ? '#10b981' : '#10b981') : (isDark.value ? '#475569' : '#cbd5e1'),
                                opacity: p.is_online ? 0.6 : 0.3
                            }
                        });
                    });
                    ...
                    const options = {
                        ...
                        groups: {
                            server: {
                                ...
                                font: { size: 16, strokeWidth: 4, strokeColor: isDark.value ? '#0f172a' : '#fff' }
                            },
                            client_online: {
                ...
                const togglePhysics = () => {
                    ...
                    networkInstance.setOptions({ physics: { enabled: physicsEnabled.value } });
                };

                const initHeatmap = () => {
                    const ctxEl = document.getElementById('trafficHeatmap');
                    if (!ctxEl || !analysisData.value?.hourly_profile) return;
                    ...
                    new Chart(ctx, {
                        ...
                        data: {
                            ...
                            datasets: [{
                                label: '综合负载',
                                data: analysisData.value.hourly_profile.map(h => h.rx_sum + h.tx_sum),
                        ...
                        options: {
                            ...
                            scales: {
                                y: {
                                    ...
                                    ticks: { color: isDark.value ? '#64748b' : '#94a3b8', callback: (v) => fmtBytes(v) },
...
`;
                ...
                const fmtRate = (v) => (v || 0).toFixed(2) + ' Mbps';
                const sort = (key) => {
                ...
                return {
                    ...
                    accessLogs, logsLoading, fetchAccessLogs, getCountryCode, getGeoTitle,
                    showIpDetailModal, selectedIpInfo, openIpInfo,
                    analysisTab, changeAnalysisTab,
                    selectedNode, physicsEnabled, resetTopologyZoom, togglePhysics,
                    showDiagnostics, diagTab, diagTarget, diagCount, diagLoading, pingResult, tracerouteResult, openDiagnostics, executePing, executeTraceroute,
                    extractEndpointIP
                };
            }
        }).mount('#app');
